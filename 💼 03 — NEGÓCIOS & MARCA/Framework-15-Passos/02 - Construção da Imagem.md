...
```mermaid
graph TD
    A[IMAGEM PESSOAL] --> B[Visual/Estética]
    A --> C[Comportamental]
    A --> D[Reputacional]
    B --> E[Foto de perfil profissional]
    B --> F[Identidade visual]
    B --> G[Cenário/ambiente]
    C --> H[Como você se comunica]
    C --> I[Como você trata as pessoas]
    C --> J[Seus valores em ação]
    D --> K[O que falam de você]
    D --> L[Provas sociais]
    D --> M[Cases e resultados]
```
